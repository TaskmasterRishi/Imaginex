...
const WEBHOOK_URL = process.env.SITE_URL ? 'https://56ff-2409-40c1-3023-a254-c24e-b5d4-89ce-3e7d.ngrok-free.app'
...
export async function POST(request: NextRequest) {
    try {
        ...
        if(!fileUrl?.signedUrl){
            return NextResponse.json({error: "Failed ot get file url"})
        }

        const modelId = `${user.id}_${Date.now()}_${input.modelName.toLowerCase().replace(" ","_")}`
